...
if (typeof window === 'undefined') {
  ...
  const loginWithTencent = async (config = {}) => {
    ...
    if (!isLoggedIn) {
      // If user needs temporary credentials without --login, provide this message
      if (!config.login) {
  ...
  const startTencentRemoteLogAndDebug = async (functionInfo, region, cliCallback) => {
    ...
    if (doesRuntimeSupportDebug(functionInfo.runtime)) {
      ...
      ws.on('error', (e) => {
        cliCallback(
          `调试错误, 错误信息:${e.message || JSON.stringify(e)}\n, websocket连接地址: ${
            tencentDebuggger.logAddr
          } \n, 开启调试模式功能参数: ${JSON.stringify(functionInfo)} 和${JSON.stringify(region)}`,
          { type: 'error' }
        );
      });
      tencentDebuggger.stopAll = async () => {
  ...
  utils.doesRuntimeSupportDebug = doesRuntimeSupportDebug;
}
